...
class MapTable(NamedTuple):
    ...
    """Bytes between the start of one data row and the next.

    ``None`` for contiguous rectangular data.  Set when the table is one
    half of a compound pair (two maps sharing a Y axis whose cells
    interleave per row) — equals ``cols_total * cell_width`` of the
...
def scan_map_tables(
    ...
    max_results:
        ...
        ``None`` for unlimited.  Defaults to ``8000`` — real 4 MB ECU
        binaries contain ~2,500+ candidates at ``min_score=0.55``, so a
        smaller cap silently truncates genuine tables.
    max_series_tables:
    ...
    return [MapTable(offset=o, cols=c, rows=r, cell_width=cw,
                    byte_order=bo, x_axis_offset=xo, y_axis_offset=yo,
                    score=s, stride=st)
            for (o, c, r, cw, bo, xo, yo, s, st) in raw]
