"""
ECU Recipe Builder — .remap format

Accepts two ECU binary files as raw bytes (in-memory), compares them and
produces a .remap recipe consumed by the patcher pipeline.
...
contains only the diff engine and .remap recipe assembly.
...
from openremap.core.services.recipes.annotator import RecipeAnnotator
...
def derive_trust_level(creator: dict) -> str:
    ...
    verified = bool(creator.get("id"))  # stable ID = platform-verified (future)
...
class ECUDiffAnalyzer:
    ...
    .remap recipe — the same format consumed by the patcher pipeline.
    ...
    def build_recipe(self, description: str | None = None) -> Dict:
        """
        Build the full .remap recipe dict.
