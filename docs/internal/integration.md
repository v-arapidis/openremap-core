> **⚠ DEPRECATED.** This documents the old `openremap-server`
> JSON-RPC protocol. The server will be replaced by a new API when
> the time comes — do not build new integrations on it. Kept here
> for reference only; it is **not** part of the public website docs.
...
The returned `recipe` dict (schema 4.5 — volatile-aware; 4.3/4.4 variants patch identically) is ready to be serialised, stored, or
...
```python
{
    "type": "recipe", "schema_version": "4.5",
...
# Scan a folder
openremap scan ./bins/ --move --organize

# Build a recipe from a diff
openremap cook stock.bin modified.bin --output tune.remap

# Validate a binary against a recipe
openremap validate before target.bin tune.remap

# Apply a recipe
openremap tune target.bin tune.remap --output patched.bin
```
